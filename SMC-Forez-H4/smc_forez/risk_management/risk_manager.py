"""Risk management module for SMC Forez - Fixed for micro accounts"""
...
class RiskManager:
    """Advanced risk management system optimized for micro accounts"""
    ...
    def __init__(self, 
        ...
        # Risk parameters by level - ADJUSTED FOR MICRO ACCOUNTS
        if initial_balance < 50:
            self.risk_params = {
                RiskLevel.CONSERVATIVE: {
                    'max_position_risk': 0.015,  # 1.5% per position for live micro accounts
                    'max_positions': 4,          # Limit to 4 positions for safety
                    'min_rr_ratio': 4.0,         # High R:R required for safety
                    'max_leverage': 2.0          # Very low leverage
                },
                RiskLevel.MODERATE: {
                    'max_position_risk': 0.02,   # 2% per position for live trading
                    'max_positions': 4,          # Maximum 4 positions
                    'min_rr_ratio': 3.0,
                    'max_leverage': 3.0
                },
                RiskLevel.AGGRESSIVE: {
                    'max_position_risk': 0.03,   # 3% per position (allows 0.03 lots)
                    'max_positions': 4,          # Single position focus for safety
                    'min_rr_ratio': 2.5,
                    'max_leverage': 3.0          # Reduced leverage for live account
                }
            }
        else:
            self.risk_params = {
                RiskLevel.CONSERVATIVE: {
                    'max_position_risk': 0.005,  # 0.5% per position for small accounts
                    'max_positions': 20,          # Demo account - allow many positions
                    'min_rr_ratio': 3.0,         # Higher R:R for small accounts
                    'max_leverage': 3.0          # Lower leverage for safety
                },
                RiskLevel.MODERATE: {
                    'max_position_risk': 0.01,   # 1% per position
                    'max_positions': 25,          # Demo account - allow many positions
                    'min_rr_ratio': 2.5,
                    'max_leverage': 5.0
                },
                RiskLevel.AGGRESSIVE: {
                    'max_position_risk': 0.01,   # 1% per position (safer for many positions)
                    'max_positions': 35,          # Demo account - accept all signals
                    'min_rr_ratio': 2.0,
                    'max_leverage': 10.0
                }
            }
    ...
    def calculate_position_size(self, 
        ...
        """Calculate optimal position size with risk management - FIXED FOR MICRO ACCOUNTS"""
        try:
            ...
            # Calculate pip value and risk - CORRECTED FOR MICRO ACCOUNTS
            ...
            # CRITICAL FIX: Micro symbols are broker's micro lots, not micro accounts
            # Symbols ending with 'm' are standard lots on micro spreads, not micro lots
            is_micro_symbol = symbol.endswith('m') or symbol.endswith('M')
            lot_size = 100000  # All forex pairs use standard lot size (despite 'm' suffix)
            
            pip_value = pip_size * lot_size
            position_size = risk_amount / (risk_pips * pip_value)
            
            # Apply size limits based on ACTUAL broker requirements discovered from MT5
            if symbol.upper() in ['GER30', 'JPN225', 'HK50', 'FRA40', 'ESP35', 'SWI20', 'NETH25', 'NOR25']:
                # These indices require 1.0 minimum lot size
                min_size = 1.0
                max_size = 50.0 if self.current_balance > 50000 else 10.0
            elif symbol.upper() in ['US30', 'UK100', 'AUS200', 'USTEC']:
                # These indices require 0.1 minimum lot size  
                min_size = 0.1
                max_size = 50.0 if self.current_balance > 50000 else 10.0
            elif symbol.upper() in ['XAUUSD', 'XAGUSD']:
                # Precious metals - 0.01 minimum
                min_size = 0.01
                max_size = 10.0 if self.current_balance > 50000 else 2.0
            else:
                # Standard forex pairs - 0.01 minimum
                if self.current_balance < 50:
                    min_size = 0.01    # Broker minimum is usually 0.01 lots
                    max_size = 0.03    # User requested maximum for $15 live account
                elif self.current_balance < 1000:
                    min_size = 0.01    # Standard minimum
                    max_size = 1.0     # Conservative for small accounts
                else:
                    min_size = 0.01   # Standard minimum
                    max_size = 100.0   # High maximum for demo testing
            
            position_size = max(min_size, min(position_size, max_size))
            
            # Round position size to 2 decimal places (broker standard)
            position_size = round(position_size, 2)
            
            # CRITICAL FIX: Check portfolio risk limits with correct risk calculation
            actual_risk_amount = position_size * risk_pips * pip_value
            portfolio_risk = self._calculate_portfolio_risk(
                symbol, position_size, actual_risk_amount
            )
            ...
            if portfolio_risk.total_risk > self.max_portfolio_risk:
                ...
                # Ensure minimum volume requirements after scaling
                min_volume = 0.01  # Broker minimum
                position_size = max(min_volume, position_size)
                
                # Round with appropriate precision after scaling
                position_size = round(position_size, 2)  # Standard 2 decimal places
                    
                actual_risk_amount *= scale_factor
            ...
            position_risk = PositionRisk(
                ...
                risk_percentage=actual_risk_amount / self.current_balance,
    ...
    def _calculate_portfolio_risk(self, 
        ...
        """Calculate portfolio-level risk metrics - FIXED FOR MICRO ACCOUNTS"""
        total_risk = new_risk
        
        # Use standard lot size for all forex pairs
        lot_size = 100000  # All forex pairs use standard lot size
        total_exposure = new_size * lot_size  # Convert to base currency units
        
        # Add existing positions
        for pos in self.open_positions.values():
            total_risk += pos.risk_amount
            # Use standard lot size for all positions
            pos_lot_size = 100000
            total_exposure += pos.position_size * pos_lot_size
    ...
    def sync_with_mt5_positions(self):
        ...
        try:
            ...
            # Remove positions that don't exist in MT5
            internal_symbols = set(self.open_positions.keys())
            stale_symbols = internal_symbols - mt5_symbols
            
            for symbol in stale_symbols:
                print(f"🧹 Removing stale position tracking for {symbol}")
                del self.open_positions[symbol]
                
            return len(stale_symbols)
            
        except Exception as e:
            print(f"⚠️ Could not sync with MT5: {e}")
            return 0
    ...
    def can_trade(self, symbol: str, signal_confidence: float = 1.0) -> Tuple[bool, str]:
        ...
        # Sync with MT5 to ensure accurate position count
    ...
    def get_risk_summary(self) -> Dict:
        ...
        # Calculate exposure with standard lot sizes
        total_exposure = 0
        for pos in self.open_positions.values():
            pos_lot_size = 100000  # All forex pairs use standard lot size
            total_exposure += pos.position_size * pos_lot_size
    ...
    def print_risk_report(self):
        ...
        if self.open_positions:
            ...
            for symbol, pos in self.open_positions.items():
                print(f"{symbol}: {pos.position_size:.4f} lots "
...
def create_risk_manager(risk_profile: str = "conservative", 
                       balance: float = 13.0) -> RiskManager:
    """Factory function to create risk manager for micro accounts"""
    risk_level_map = {
    ...
    # Adjust risk limits - Demo account allows higher risk for testing
    max_portfolio_risk = 0.02 if balance < 100 else 0.20  # 20% for demo testing
    max_daily_loss = 0.01 if balance < 100 else 0.10      # 10% daily loss limit
